...
def randomSlopes_NUTS(num_warmup=1000, num_samples=1000, num_chains=2, rng_seed=0):
    ...
    file_path = os.path.join("simplicity_bias", "data","glme_numpyro_"+str(num_warmup)+"_"+str(num_samples)+"_null_seed"+str(rng_seed)+".pickle")
...
def powerlaw_logit_model_null(y, V, invN, dD_raw, dTrain_z, subj_idx, S):
    ...
    # Slopes on ΔtrainMSE
    mu_t    = numpyro.sample("mu_t",    dist.Normal(1.0, 1.0))
...
randomSlopes_NUTS(num_warmup=2000, num_samples=2000, num_chains=4, rng_seed=1)